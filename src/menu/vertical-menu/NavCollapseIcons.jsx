      ...
      {isBreakpointReached ? (
        <span role='button' tabIndex={0} style={{ display: 'flex', cursor: 'pointer' }} onClick={handleClose} {...rest}>
          {closeIcon ?? <CloseIcon />}
            ...
            <span role='button' tabIndex={0} style={{ display: 'flex', cursor: 'pointer' }} onClick={() => handleClick('lock')} {...rest}>
              {unlockedIcon ?? <RadioCircleIcon />}
            </span>
            ) : (
            <span role='button' tabIndex={0} style={{ display: 'flex', cursor: 'pointer' }} onClick={() => handleClick('unlock')} {...rest}>
